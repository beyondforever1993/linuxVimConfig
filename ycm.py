...
if __name__ == '__main__':
    ...
    ycm_strio.write(ycm_content_head)
    ...
    for root, dirs, files in os.walk(work_dir, followlinks = True):
        for dirname in dirs:
            ...
            for ig_dir in ignore_dirs:
                if dir_path.find(ig_dir, 0, len(dir_path)) >= 0:
                    match_ig_dir = True;
                    break;
                    
            if match_ig_dir == False:
                ycm_strio.write('\'-I\', ')
                ycm_strio.write('\'%s\',\n' % dir_path)

    ycm_strio.write(ycm_content_tail)

    with open('.ycm_extra_conf.py', 'w') as ycm_fd:
        ycm_fd.write(ycm_strio.getvalue())

    print('generate ycm file success')
